...
/**
 * Trouve l'URL de connexion, quel que soit le nom donné par l'hébergeur.
 * Neon via le Marketplace Vercel injecte selon les cas DATABASE_URL,
 * POSTGRES_URL, DATABASE_URL_UNPOOLED… On ne devine pas : on cherche.
 */
export function urlBase() {
  ...
  for (const k of prioritaires) {
    if (process.env[k]?.startsWith('post')) return { url: process.env[k], nom: k };
  }
  // Filet de sécurité : n'importe quelle variable contenant une URL Postgres.
  for (const [k, v] of Object.entries(process.env)) {
    if (typeof v === 'string' && /^postgres(ql)?:\/\//.test(v)) return { url: v, nom: k };
  }
  return { url: '', nom: '' };
}
...
async function pilote() {
  ...
  const { url } = urlBase();
  if (!url) throw new Error('Aucune URL Postgres trouvée dans les variables d’environnement.');
...
export const baseDisponible = () => !!urlBase().url;
